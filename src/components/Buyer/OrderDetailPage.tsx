...
const fmtMoney = (p: number) =>
  `${new Intl.NumberFormat("vi-VN").format(Number(p) || 0)} VND`;
...
> = {
  PENDING_PAYMENT: {
    ...
    bg: "#fffbeb",
    icon: <Clock size={14} />,
    desc: "Đang chờ xử lý thanh toán",
  },
  ESCROWED: {
    ...
    bg: "#eff6ff",
    icon: <Clock size={14} />,
    desc: "Đơn hàng của bạn đã được tạo, đang chờ seller xác nhận",
  },
  ACCEPTED: {
    label: "Seller đã xác nhận",
    color: "#8b5cf6",
    bg: "#f5f3ff",
    icon: <CheckCircle size={14} />,
    desc: "Seller đã nhận đơn, đang chuẩn bị giao hàng",
  },
  SHIPPED: {
    ...
    bg: "#f0f9ff",
    icon: <Truck size={14} />,
    desc: "Seller đã gửi hàng cho đơn vị vận chuyển",
  },
  DELIVERED: {
    label: "Đã giao hàng",
    color: "#f59e0b",
    bg: "#fffbeb",
    icon: <Truck size={14} />,
    desc: "Seller đã giao hàng, vui lòng xác nhận khi nhận được",
  },
  COMPLETED: {
    label: "Đã hoàn thành",
    color: "#10b981",
    bg: "#f0fdf4",
    icon: <CheckCircle size={14} />,
    desc: "Giao dịch hoàn tất, tiền đã về tay seller",
  },
  CANCELLED: {
    label: "Đã hủy",
    color: "#ef4444",
    ...
    desc: "Bạn đã hủy đơn hàng, tiền đã hoàn về ví bạn",
  },
  SELLER_CANCELLED: {
    label: "Người bán đã hủy đơn hàng",
    color: "#ef4444",
    bg: "#fef2f2",
    icon: <XCircle size={14} />,
    desc: "Người bán đã hủy đơn hàng, tiền đã hoàn về ví bạn",
  },
  REFUNDED: {
    label: "Đã hoàn tiền",
    color: "#10b981",
    ...
    desc: "Tiền đã được hoàn về ví của bạn",
  },
  RETURN_REQUESTED: {
    ...
    bg: "#fffbeb",
    icon: <RotateCcw size={14} />,
    desc: "Đang chờ seller xác nhận nhận lại hàng",
  },
  DISPUTED: {
    ...
    desc: "Admin đang xử lý tranh chấp giữa hai bên",
  },
  UNKNOWN: {
    ...
    bg: "#f8fafc",
    icon: <Clock size={14} />,
    desc: "Hệ thống vừa cập nhật trạng thái đơn hàng",
  },
};
...
export default function OrderDetailPage() {
  ...
  // ====== STATE ĐẾM NGƯỢC TỰ ĐỘNG XÁC NHẬN ĐƠN HÀNG ======
  // countdown: lưu thời gian còn lại (ngày, giờ, phút, giây) cho đến khi hệ thống tự động xác nhận nhận hàng
  // countdownRef: tham chiếu đến interval để clear khi cần
  ...
  // ====== HÀM CHUYỂN ĐỔI GIÂY SANG NGÀY, GIỜ, PHÚT, GIÂY ======
  // Dùng để hiển thị countdown cho người dùng
  ...
  //  "DELIVERED", hệ thống đếm ngược thời gian tự động xác nhận nhận hàng
  //  Khi = 0 tự động gọi API xác nhận nhận hàng và reload
  useEffect(() => {
    ...
    if (
      ...
      totalSeconds =
        detail.order.remainingDays * 86400 +
        detail.order.remainingHours * 3600 +
        detail.order.remainingMinutes * 60 +
        detail.order.remainingSeconds;
    ...
    if (totalSeconds > 0) {
      countdownRef.current = setInterval(() => {
        ...
        if (totalSeconds <= 0 && countdownRef.current) {
          ...
          // GỌI API XÁC NHẬN NHẬN HÀNG TỰ ĐỘNG KHI COUNTDOWN = 0 
          (async () => {
            try {
              await confirmReceiptAPI(detail.order.id);
            } catch (e) {
              alert(
                "Lỗi xác nhận tự động: " + (e instanceof Error ? e.message : e),
              );
            } finally {
              fetchDetail();
            }
    ...
    // ====== CLEAR INTERVAL KHI RỜI KHỎI TRẠNG THÁI DELIVERED HOẶC UNMOUNT ======
    return () => {
      if (countdownRef.current) clearInterval(countdownRef.current);
    };
  ...
  const submitOpenDispute = async (payload: OpenDisputePayload) => {
    if (!orderId) return;
    await doAction("return-dispute", () =>
      openReturnDisputeAPI(orderId, payload),
    );
    ...
    return (
      ...
      >
        ...
        >
          ...
          <button
            onClick={() => navigate("/buyer", { state: { tab: "orders" } })}
            style={{
              padding: "10px 24px",
              background: "#2563eb",
              color: "white",
  ...
  return (
    ...
    >
      ...
      <div
        className="od-wrap"
        style={{ maxWidth: 760, margin: "0 auto", padding: "28px 20px 64px" }}
      >
        {/* ── Status hero ── */}
        <div
          style={{
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <p style={{ fontSize: 18, fontWeight: 800, color: meta.color }}>
                {meta.label}
              </p>
                    ...
                    {i < TIMELINE_STEPS.length - 1 && (
                      <div
                        style={{
                          flex: 1,
                          height: 2,
                          background: i < stepIdx ? "#10b981" : "#e8ecf4",
                          margin: "0 4px 16px",
          ...
          {isCancelledOrRefunded && (
            ...
            >
              {meta.icon}
              <span
                style={{ fontSize: 13, color: meta.color, fontWeight: 600 }}
              >
                {meta.desc}
              </span>
            </div>
        ...
        >
          ...
          {[
            {
              label: "Xe đạp",
              value: order.bikeTitle,
              link: `/bikes/${order.bikeId}`,
            },
            {
              label: "Người bán",
              value: order.sellerName,
              link: `/sellers/${order.sellerId}`,
            },
            {
              label: "Số tiền (VND)",
              value: fmtMoney(order.amountPoints),
              highlight: true,
            },
            ...
            order.acceptedAt
              ...
              : null,
            order.deliveredAt
              ? { label: "Đã giao hàng", value: fmtDateTime(order.deliveredAt) }
              : null,
            order.shippingCarrier
              ? { label: "Đơn vị vận chuyển", value: order.shippingCarrier }
              : null,
            order.trackingCode
            ...
            order.shippingNote
              ...
              : null,
            order.returnReason
              ? { label: "Lý do hoàn", value: order.returnReason, warn: true }
              : null,
            // ====== HIỂN THỊ ĐẾM NGƯỢC TỰ ĐỘNG XÁC NHẬN ĐƠN HÀNG ======
                    ...
                    {row.link ? (
                      ...
                      <span
                        style={{
                          ...
                          fontWeight: 600,
                          color: (
                            row as { highlight?: boolean; warn?: boolean }
                          ).highlight
                            ? "#2563eb"
            ...
            >
              {(() => {
                ...
                const hasMultipleCompleted =
                  timeline.filter((e) => e.status === "COMPLETED").length > 1;
                return timeline.map((evt, i) => {
                  const evtEffectiveStatus =
                    evt.action === "seller_cancelled"
                      ? "SELLER_CANCELLED"
                      : evt.status;
                  let evtMeta =
                    STATUS_META[evtEffectiveStatus] ??
        ...
        {(() => {
              ...
              >
                ...
                >
                  {order.status === "ESCROWED"
                    ...
                    : "Đơn hàng đã được seller xác nhận. Bạn vẫn có thể hủy đơn trước khi seller giao hàng."}
                </p>
                <button
          ...
          if (order.status === "DELIVERED")
            return (
              ...
              >
                ...
                >
                  ⏱ Hàng đã được giao. Xác nhận nhận hàng để giải ngân tiền cho
                  seller, hoặc yêu cầu hoàn hàng nếu có vấn đề.
                  {countdown && order.status === "DELIVERED"
          ...
          if (order.status === "RETURN_REQUESTED")
            return (
              ...
              >
                ...
                >
                  Đang chờ seller xác nhận nhận lại hàng. Nếu seller không phản
                  hồi, bạn có thể mở tranh chấp.
                </p>
                <button
                  className="action-btn"
          ...
          if (order.status === "COMPLETED" && detail?.canReview)
            return (
              <button
                className="action-btn"

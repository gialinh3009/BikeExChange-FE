...
export default function OrderApprovalModal({ isOpen, order, token, onClose, onSuccess }: OrderApprovalModalProps) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, fontFamily: "'DM Sans',sans-serif" }}>
      <div style={{ background: "white", borderRadius: 18, border: "1.5px solid #e8ecf4", padding: 28, maxWidth: 420, width: "90%", boxShadow: "0 20px 60px rgba(0,0,0,.15)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: "#eff6ff", display: "flex", alignItems: "center", justifyContent: "center", color: "#2563eb" }}><CheckCircle size={20} /></div>
        ...
        <div style={{ background: "#f8faff", borderRadius: 12, padding: 14, marginBottom: 20 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, fontSize: 12 }}>
            ...
            <div><p style={{ color: "#94a3b8", marginBottom: 4 }}>Ngày tạo</p><p style={{ fontWeight: 600, color: "#0f172a" }}>{fmtDateTime(order.createdAt)}</p></div>
          </div>
        </div>
        ...
        <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 10, padding: 12, marginBottom: 20, display: "flex", gap: 10 }}>
          ...
          <p style={{ fontSize: 12, color: "#047857", margin: 0, lineHeight: 1.5 }}>Khi xác nhận, bạn cam kết sẽ giao hàng cho buyer. Tiền sẽ được giữ cho đến khi buyer xác nhận nhận hàng.</p>
        </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button onClick={() => void handleApprove()} disabled={loading} style={{ flex: 1, padding: "12px 16px", background: loading ? "#cbd5e1" : "#2563eb", color: "white", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
            {loading ? <>

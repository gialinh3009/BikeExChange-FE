...
export default function CreateBikeTab({ token, wallet, onBikeCreated, onWalletRefresh }: CreateBikeTabProps) {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const rawCats = Array.isArray(resCats)
                    ? resCats
                    : (isObject(resCats) && "data" in resCats ? (resCats as ApiDataWrapper<unknown>).data : []);
                setCategories((Array.isArray(rawCats) ? rawCats : []).filter(isCategory));
                const rawBrands = Array.isArray(resBrands)
                    ? resBrands
                    : (isObject(resBrands) && "data" in resBrands ? (resBrands as ApiDataWrapper<unknown>).data : []);
    ...
    const handleSubmit = async () => {
        ...
        if (!form.title || !form.brandId || !form.priceVnd) {
        ...
        try {
            ...
            setError(e?.message || "Không thể tạo xe.");
        } finally {
            setLoading(false);
            setUploading(false);
        }
            ...
            <div className="p-6 space-y-6">
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className={`text-sm ${hasEnough ? "text-emerald-700" : "text-red-700"}`}>
                                {hasEnough
                                    ? `Ví đủ tiền. Hệ thống sẽ tự động trừ ${POSTING_FEE.toLocaleString("vi-VN")} VND khi đăng.`
                ...
                <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <span className="ml-3 text-sm text-gray-600">
                        Đã chọn: <b className="text-blue-600">{images.length}</b> ảnh
                    </span>
                    {images.length > 0 && (
                ...
                <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="grid gap-5 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <select
                                value={form.brandId ?? ""}
                                onChange={(e) => setForm((p) => ({ ...p, brandId: e.target.value ? Number(e.target.value) : undefined }))}
                                className="w-full rounded-xl border-2 border-gray-200 px-4 py-3 text-sm outline-none focus:border-blue-500 bg-gray-50 focus:bg-white"
                            >
                    ...
                    <div className="mt-5">
                        ...
                        <select
                            ...
                            onChange={(e) => setForm((p) => ({ ...p, categoryId: e.target.value ? Number(e.target.value) : undefined }))}
                            className="w-full rounded-xl border-2 border-gray-200 px-4 py-3 text-sm outline-none focus:border-blue-500 bg-gray-50 focus:bg-white"
                        >
                ...
                <div className="flex justify-end gap-3">
                    ...
                    <button
                        ...
                        disabled={loading || uploading || !hasEnough}
                        ...
                        <div className="bg-gradient-to-r from-emerald-50 to-emerald-100 px-6 py-8 text-center">
                            ...
                            <div className="bg-white rounded-xl p-4 mb-4">
                                ...
                                <div className="text-2xl font-bold text-emerald-600">
                                    {POSTING_FEE.toLocaleString("vi-VN")} VND
                                </div>
                            </div>

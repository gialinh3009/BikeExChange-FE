...
export async function getAdminInspectorsAPI({ page = 0, size = 10 } = {}) {
  ...
  if (!res.ok || !data.success)
    throw new Error(data.message || "Không thể tải danh sách kiểm định viên.");
  return data;
}

/**
 * Lấy danh sách báo cáo kiểm định (Admin)
 */
export async function getAdminInspectionReportsAPI({ page = 0, size = 20 } = {}) {
  ...
  const data = await res.json();
  if (!res.ok || !data.success)
    throw new Error(data.message || "Không thể tải danh sách báo cáo kiểm định.");
  return data;
}

/**
 * Lấy danh sách yêu cầu kiểm định theo trạng thái
 * status: REQUESTED | ASSIGNED | INSPECTED | APPROVED | REJECTED
 */
export async function getInspectionsByStatusAPI({ status, page = 0, size = 20 } = {}) {
  ...
  const data = await res.json();
  if (!res.ok || !data.success)
    throw new Error(data.message || "Không thể tải danh sách kiểm định.");
  return data;
}
...
export async function approveInspectionAPI(inspectionId, result) {
  const res = await fetch(
    `${BASE_URL}/inspections/${inspectionId}/approve?result=${result}`,
    {
      method: "POST",
      headers: authHeaders(),
    }
  ...
  const data = await res.json();
  if (!res.ok || !data.success)
    throw new Error(data.message || "Không thể xử lý kiểm định.");
  return data;
}